...
def get_kp_f107(dates):
    ...
    for cdate in dates:                                    # for each date to pull info for, open database file
      with open(path.join(args.path, 'KP_AP_F107', cdate[:4])) as file:
        for line in file:                                  # for each line
          if line[:6] == hourless(cdate[2:]):              # match date
                                                           # append to lists
  ...
  return interpolate(kp, mins_per_kp_segment), interpolate(f107, mins_per_f107_segment)
...
def time_diff(start, compare):
  ...
  return (start_time-comp_time).seconds/60
...
def parse(start_date, end_date, hduration):
  ...
  kp, f107 = get_kp_f107(get_dates(min_f107, max_f107))
  kp_avg   = get_24hr_kp_avg(get_dates(start_date,end_date))
  hemi_pow = get_hemispheric_power(get_dates(start_date,end_date))

  # and select the right ones
  kp_offset     = time_diff(start_date, hourless(min_f107) + kp_midpoint_string)
  f107_offset   = time_diff(start_date, hourless(min_f107) + f107_midpoint_string)
  kp_avg_offset = time_diff(start_date, hourless(start_date) + '0000')
  hemi_offset   = kp_avg_offset

  # return our subarray
  return kp[kp_offset:kp_offset+mduration], f107[f107_offset:f107_offset+mduration], \
         kp_avg[kp_avg_offset:kp_avg_offset+mduration], hemi_pow[hemi_offset:hemi_offset+mduration]
...
def output(file, start_date, kp, f107, kp_avg, hemi_pow):
  ## simply
  f = open(file,'w')
  ...
  f.write(' Date_Time                   F10          Kp   24HrKpAvg     HemiPow     F10Flag      KpFlag   \n')
  f.write('--------------------------------------------------------------------------------------------   \n')
  for i,j in enumerate(kp):
    f.write('{0}{1:12.7f}{2:12.7f}{3:12.7f}{4:12.7f}{5:>12}{6:>12}'.format( \
...
def run(start_date, duration):
  ## we do it
  end_date = new_timestamp(start_date, duration)
  kp, f107, kp_avg, hemi_pow = parse(start_date, end_date, duration)
  output('test', start_date, kp, f107, kp_avg, hemi_pow)
...
parser.add_argument('-s', '--start_date', help='starting date of run (YYYYMMDDhh)', required=True, type=str)
parser.add_argument('-p', '--path',      help='path to database files', required=True, type=str)
...
midpoint_f107          = 15*60
midpoint_f107_fraction = midpoint_f107/mins_per_f107_segment
f107_midpoint_string   = '1500'
kp_midpoint_string     = '0130'

## __MAIN__
run(args.start_date, args.duration)
