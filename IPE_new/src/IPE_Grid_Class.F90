...
!ENDDO
  ...
  SUBROUTINE Interpolate_2D_to_Geographic_Grid( grid, apex_data, geographic_data )
    IMPLICIT NONE
    CLASS( IPE_Grid ), INTENT(in) :: grid
    REAL(prec), INTENT(in)        :: apex_data(1:grid % NLP,1:grid % NMP)
    REAL(prec), INTENT(out)       :: geographic_data(1:nlon_geo,1:nlat_geo)
    ! Local
    INTEGER    :: l, m, lp, mp 
    INTEGER    :: i, iheight, npts
    INTEGER    :: in1, in2, lp1, lp2, iFlux1, iFlux2
        ...
        DO l = 1, nlat_geo
          DO m = 1, nlon_geo
            ...
            DO i=1,3
              ...
              data_interpolated = ((apex_data(lp2,mp)-apex_data(lp1,mp))*factor) + apex_data(lp1,mp)

              dtotinv = (1.0_prec/grid % dd_interface(i,iheight,l,m)) + dtotinv

              geographic_data(m,l) = (data_interpolated/grid % dd_interface(i,iheight,l,m)) + geographic_data(m,l)

            ENDDO

            geographic_data(m,l)   = geographic_data(m,l)/dtotinv
  ...
  END SUBROUTINE Interpolate_2D_to_Geographic_Grid
