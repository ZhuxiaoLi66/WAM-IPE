...
!--------------------------------------------  
      ...
      USE module_FIELD_LINE_GRID_MKS, ONLY: ON_m3, O2N_m3, N2N_m3, HN_m3, HE_m3, N4S_m3, TN_k, TINF_k, Un_mag_ms1, &
                                            Vn_geo_ms1_4output
...
!---------------------------
      ...
      REAL(KIND=real_prec), dimension(3,IPDIM) :: Vn_geo_ms1

! Neutral Wind vectors:
! Vn_geo_ms1 : Neutral Wind on geographic frame:  1:east; 2:north; 3:up
! Un_mag_ms1 : Neutral Wind on geomagnetic frame: 1:east; 2:down/equatorward; 3:parallel (South to North)
...
! Dummy variables for when IPE is coupled to WAM.....
      ...
      REAL (KIND=real_prec),dimension(3,IPDIM) :: Vn_geo_ms1_dummy
...
Vn_geo_ms1(1,IN-IN+1:ihTopN-IN+1)   =  WamField(IN:ihTopN,lp,mp,2)      !Vn_geographic_east < 800km NH
Vn_geo_ms1(1,ihTopS-IN+1:IS-IN+1)   =  WamField(ihTopS:IS,lp,mp,2)      !Vn_geographic_east < 800km SH
Vn_geo_ms1(2,IN-IN+1:ihTopN-IN+1)   =  WamField(IN:ihTopN,lp,mp,3)      !Vn_geographic_north < 800km NH
Vn_geo_ms1(2,ihTopS-IN+1:IS-IN+1)   =  WamField(ihTopS:IS,lp,mp,3)      !Vn_geographic_north < 800km SH
Vn_geo_ms1(3,IN-IN+1:ihTopN-IN+1)   =  WamField(IN:ihTopN,lp,mp,4)      !Vn_geographic_up < 800km NH
Vn_geo_ms1(3,ihTopS-IN+1:IS-IN+1)   =  WamField(ihTopS:IS,lp,mp,4)      !Vn_geographic_up < 800km SH
...
Vn_geo_ms1(1,ipts)                  =  WamField(ihTop,lp,mp,2)          !Vn_geographic_east > 800km 
Vn_geo_ms1(2,ipts)                  =  WamField(ihTop,lp,mp,3)          !Vn_geographic_north > 800km 
Vn_geo_ms1(3,ipts)                  =  WamField(ihTop,lp,mp,4)          !Vn_geographic_up > 800km   
...
! Write the geographic wind across to a 3-D parameter for output....
                   ...
                   do jth=1,3
                     do i=IN,IS
                       Vn_geo_ms1_4output(i-IN+1,lp,mp,jth)=Vn_geo_ms1(jth,i-IN+1)
                     end do
                   end do
...
! the plasma parameters file....
                   ...
                   do jth=1,3
                     do i=IN,IS
                       Vn_geo_ms1(jth,i-IN+1) = Vn_geo_ms1_4output(i-IN+1,lp,mp,jth)
                     end do
                   end do
...
! un(3)=Ue3=d3*U: positive parallel to a field line, Eq(5.6) 
               ...
               IF ( dotprod > 0.0 ) THEN
                  Un_mag_ms1(i,lp,mp,3) = & 
                       &     ( apexD(i,lp,mp,east ,3)*Vn_geo_ms1(1,ipts)     &
                       &     + apexD(i,lp,mp,north,3)*Vn_geo_ms1(2,ipts)     &
                       &     + apexD(i,lp,mp,up   ,3)*Vn_geo_ms1(3,ipts) ) / &
                       &     SQRT(  dotprod   )
               ELSE
                  Un_mag_ms1(i,lp,mp,3) = 0.0
               END IF

!dbg20110131: the midpoint values become NaN otherwise because of inappropriate D1/3 values...
               IF ( lp>=1 .AND. lp<=6 .AND. i==midpoint )   Un_mag_ms1(i,lp,mp,:) = Un_mag_ms1(i-1,lp,mp,:) 
