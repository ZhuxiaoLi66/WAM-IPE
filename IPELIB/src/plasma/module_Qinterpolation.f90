...
! factor2 IS NOT equal for all mp
        ...
        ENDDO flux_tube_loopT0 !: DO i=IN,IS

      ENDIF ! IF ( plasma_grid_3d( JMIN_IN(lp0) , lp0,mp0,IQ) < plasma_grid_3d(ip ,lp,mp,IQ) ) THEN
...
!SMS$IGNORE end
      ...
      ENDIF !(ispecial == 1) THEN
    ENDDO flux_tube_loopT1_Q !: DO ip=IN,IS
