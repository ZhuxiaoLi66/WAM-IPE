...
CONTAINS
  ...
  SUBROUTINE stepback_mag_TH (utime_local,mp,lp,phi_t0,theta_t0,r0_apex)
    ...
    USE module_input_parameters,ONLY: time_step,sw_exb_up,start_time,lpmin_perp_trans,fac_exb_up, sw_perp_transport,sw_convection_footpoint_0_or_apex_1,STOP_time,mype, perp_transport_time_step
    IMPLICIT NONE
! INPUT
...
!
    ...
    DO ihem=1,1 !ihem_max  !<< Do we need this loop ?
      ...
      IF ( sw_convection_footpoint_0_or_apex_1==1 ) THEN !R method (gip)
      ...
      ELSE IF ( sw_convection_footpoint_0_or_apex_1==0 ) THEN !th method (ctipe/shawn) ! Usually evaluates here
