...
C-----------------------------------------------------------------------
      SUBROUTINE GRIBIT_GSM(gribi,LBM,IDRT,IM,JM,MXBIT,COLAT1,
...
C   13-02-22  Wang     change name to gribit_gsm.f from gribit.f to avoid
!                      duplicate name from post
C
...
!!
     ...
     &                     orient,dely,delx
      ...
      LOGICAL(1) LBM(IM*JM)
      CHARACTER GRIB(*)
      INTEGER IBMARY(IM*JM*IBMS+1-IBMS),IPDS(100),IGDS(100),IBDS(100)
...
csela  write(0,*) 'gribit getbit'
      IF(NBM.EQ.0) THEN
        ...
        NBIT=0
      ELSE
        CALL GETBIT(IPDS(7),0,IDS,NF,IBMARY,F,FR,FMIN,FMAX,NBIT)
