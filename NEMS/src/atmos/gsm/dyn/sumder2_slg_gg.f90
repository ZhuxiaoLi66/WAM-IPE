...
!
      ...
      use gfs_dyn_resol_def , only : jcap,latgd
      use gfs_dyn_layout1   , only : len_trie_ls,len_trio_ls,
     &                               ls_dim,ls_max_node,me,nodes
      use gfs_dyn_mpi_def   , only : kind_mpi,mpi_r_mpi,mc_comp
...
!    ------------
!
      real(kind=kind_evod), dimension(latl2,2,nvars) :: apev, apod
...
!
      real(kind=kind_mpi), dimension(2,nvars,ls_dim*workdim,nodes) ::
     &                                works, workr
!
      integer, dimension(jcap+1) :: kpts, kptr, sendcounts, recvcounts
     &,                             sdispls
!
      integer            ierr,ilat,ipt_ls,lmax,lval,i,jj,node,nvar,lonl
...
!
     ...
     &                   mc_comp,ierr)
!
!$omp parallel do private(j,lat,lmax,nvar,lval,n2,lonl)
      do j=1,lats_node
         lat  = global_lats(ipt_lats_node-1+j)
         lonl = lons_lat(lat)
