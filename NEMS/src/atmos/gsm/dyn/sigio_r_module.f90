...
!   2012-10-18  Jun Wang --update to work for big endian files on different
!   machine
!
! Modules Used:
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    if(iret.ne.0) iret=-1
!
!-- decide do_byteswap, file endian is big_endian
    if(trim(machine_endian)=='') then
...
!    print *,'in sigio_rropen,cfname=',trim(cfname),'machine_endian=',machine_endian, &
...
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    iskip=0
    iread=sigio_lhead1
!   print *,'in sigio_rrhead,do_byteswap=',do_byteswap
    call bafrreadl(lu,iskip,iread,nread,head1a)
    if(nread.lt.iread) return
!LLF+PM--
    if(do_byteswap)call byteswap(head1a%ivs,sigio_intkind,6)
!LLF+PM==
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
!LLF+PM==
      ...
      if(nread.lt.iread) return
!LLF+PM--
      if(do_byteswap) call byteswap(head%vcoord,sigio_realkind,iread/4)
!LLF+PM==
      iskip=iskip+nread
...
!
      if (mod(head%idvm/10,10) == 3) then
        ...
        if(nread.lt.iread) return
        if(do_byteswap) call byteswap(head%cpi,sigio_realkind,iread/4)
        iskip=iskip+nread
        iread=4*size(head%ri)
        call bafrreadl(lu,iskip,iread,nread,head%ri)
        if(nread.lt.iread) return
        if(do_byteswap) call byteswap(head%ri,sigio_realkind,iread/4)
      endif
      head%clabsig=' '
...
!LLF+PM==
      ...
      iwrite=head%lhead(2)
!LLF+PM--
      if(do_byteswap) call byteswap(head2a,sigio_intkind,iwrite/sigio_intkind)
!LLF+PM==
