...
!       mindetam = max(mindetam, 0.0005)
!       mindetam = max(mindetam, 0.0001)
!
        k = plev + 1
!       leps = 1.e-05
        leps = 1.e-08
...
!         write(1000,*)' calling for her_xyz_in_h td3_a lan=',lan
          call her_xyz_in_h(i_1,i_2,s_lat_in_pe,n_lat_in_pe,
...
!           if (jtem < 0) write(1000+me,*)' jtem=',jtem,' phimp_a=',
!    & phimp_a(i,k),' i=',i,' k=',k,' phibs=',phibs,' dphibr=',dphibr
! Commented by Moorthi on 10/18/2011
!        if (phimp_a(i,k) < -900.0) then
!        write(999,*) 
...
!     if (abs(fdp(i,plevp-k)) >  1000.0)
!    &write(1000,*)' fdp=',fdp(i,plevp-k),' ht=',ht(i,k),dht(i,k)
!    &,' hb=',hb(i,k), dhb(i,k),finty(2),finty(3)
!    &,' ftop=',ftop(1),' fbot=',fbot(1),'  k=',k,' i=',i
