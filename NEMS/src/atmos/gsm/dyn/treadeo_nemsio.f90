      ...
      SUBROUTINE TREADEO_nemsio(cfile,IDATE,
     &                          trie_ls,trio_ls,
...
!
      stime = timef()
      if (me == 0) write(0,*)' before nemsio_open cfile=',trim(cfile)
...
!
     ...
     &                        ncldt=ncldt,tlmeta=tlmeta)
      ...
      if (me == 0) write(0,*)' after nemsio_getfilehead levsi=',levsi,
     &                         latgi,lonfi,' me=',me
       idate(1)   = idate7(4)
...
!       if (slg_flag) then
!         write(0,*)' before  grid_to_spect_inp_slg lotls=',lotls
...
!       else
          call grid_to_spect_inp (zsg,psg,uug,vvg,ttg,rqg,dpg,
     &                            trie_ls,trio_ls,
     &                            ls_node,ls_nodes,max_ls_nodes,
     &                            lats_nodes_a,global_lats_a,lonsperlat,
     &                            epse,epso,plnew_a,plnow_a,
...
!       endif
        ...
        if( .not. ndslfv .or. .not. gg_tracers ) then
          do k=1,levh
