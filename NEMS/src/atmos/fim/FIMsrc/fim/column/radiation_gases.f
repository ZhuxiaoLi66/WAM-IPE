...
!
     ...
     &,                                   timeozc => timeozc
      ...
      use module_control,          only : curve,NumCacheBlocksPerPE
     & ,alt_topo,gfsltln_file,mtnvar_file,aerosol_file,co2_2008_file
     & ,co2_glb_file
!
      implicit   none
!
...
!  ---  parameter constants for gas volume mixing ratioes
      ...
      real (kind=kind_phys), parameter :: co2vmr_def = 390.0e-6
...
!     data  cfile1 / 'co2historicaldata_2004.txt' /
      ...
      cfile0=trim(co2_glb_file)
      cfile1=trim(co2_2008_file)
!===>  ...  begin here

      ico2flg = ICO2

!  ---- Default value for co2_glb: co2vmr_def
...
!  --- ... check to see if requested co2 data file existed
     ...
     &                ' not found, check for other available data set'
              ...
              if ( .not. file_exist ) then
! jbao     new gfs don't have file yet
                if ( me == 0) print *,' Can not find co2 data source file, 
     . use default = ',co2_glb
              return
! end jbao
                if ( me == 0 ) then
