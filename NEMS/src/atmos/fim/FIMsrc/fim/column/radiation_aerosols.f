...
!
     ...
     &                         con_t0c, con_c, con_boltz, con_plnk
      ...
      use module_control  ,only: curve,NumCacheBLocksPerPE,alt_topo              
     &                          ,mtnvar_file,gfsltln_file,aerosol_file           
     &                          ,co2_2008_file,co2_glb_file
!
      implicit   none
!
      private
...
!     write(6,*)'take sw parms from chem routines'
!               do i = 1, IMAX
!                 do k = 1,im
!                   do m=1,nbands
!                     aerosw(i,k,m,1) = 
...
!       if ( me == 0 ) then
!         print *,'   Opened aerosol data file: ',trim(aerosol_file)
!       endif
      else
        print *,'    Requested aerosol data file "',aerosol_file,       &
     &          '" not found!'
      return
!jbao newgfs        print *,'    *** Stopped in subroutine AERINIT !!'
!jbao newgfs        stop
      endif              ! end if_file_exist_block
...
!  ---  conversion constants
! jbao new gfs phys      real (kind=kind_phys), parameter :: dltg = 360.0 / float(IMXAE) ! imxae=72
! jbao new gfs phys      real (kind=kind_phys), parameter :: hdlt = 0.5 * dltg
      real (kind=kind_phys), parameter :: dltg = 360.0 / 72.0
      real (kind=kind_phys), parameter :: hdlt = 180.0/ 72.0
