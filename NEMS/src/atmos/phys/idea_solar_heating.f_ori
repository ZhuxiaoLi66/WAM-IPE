...
!               c) idea_solar_init.f    - all time-inv fixed data and updated eff.
...
!            idea_dissociation
!            srb_jo2_heat, src_jo2_heat, lya_jo2_heat
!            get_slantcolumns
!==================================================================================
...
!
!VAY-2016
      real, intent(in)    :: f107, f107d, Kp  ! solar-geo drivers for current timestep
!
...
!
! vay-2015 remove f107/f107a   now values passed by " use idea_solar_input "
!
...
! OK radians !!!!     
!      print *, 'VAY-maglat ', maxval(maglat), minval(maglat) 
!
      do i=1,im
        do k=1,levs
          ...
          ho2(k)=.5*ho(k) 
          hn2(k)=ron2*ho(k)  
!          ho(k)=1.e3*rgas*t(k)/(amo*grav(i,k))          
          ht(k) =  zg(i,k)
          alt(k) = 1.e-3*ht(k)
          prr(k)=prsl(i,k)
          nn(k)=o(k)+o2(k)+o3(k)+n2(k)
          amm(k)=am(i,k)*vay_avgd                  ! normal mol weight 29-17 compute it in idea_phys.f
                                                   !  & pass it to all RAD-subs am2d(k)=am(i,k)*vay_avgd 
        enddo
!                  
! get heating
...
!
       ...
       use idea_solar,       only  : euv37, nsp_euv        ! current EUV-flux(nsp_euv=37)
!       use idea_solar_input, only  : idea_solar_fix 
       use idea_solar,       only  : o2_scale_factor,  SRBEFF  ! (levs)-arrays
!
       use idea_solar,       only  : effuv, effeuv             ! from CTIP-originally
...
!-------------------------------------------------------------------------
!  **
!  calculates solar heating from EUV and UV(SRC+LYA+SRB)
!  
!  Input:
...
!
      ...
      real, intent(out)   :: O2dissociation_rate(np)     ! photo dissociation rate
      real, intent(out)   :: O3dissociation_rate(np)     ! photo dissociation rate
!
! locals 
!
!
...
! Solar variability factor for SRB dissociation calculation
      ...
      REAL fmxfmn
      REAL pind
      real, dimension(np) :: sco2, sco, scn2,sco3     ! slant columns o2/o/n2 in .. 1/cm2
...
! Constants as functions of F107
!
!                                         ! eccentric=(1.0+0.0167*COS(2.0*pi*(dayno-3)/366.0))**2=sfeps
!vay-2017        fmxfmn=(F107-71.)/(220-71.)       ! does not make sense for F107~70.
        fmxfmn=(f107-65.)/165.   
!                                         ! fmxfmn=(f107d-65.)/165.      !TIMEGCM/WX o2srbc.F
        srband_fac=0.784591675
        vay_fmxfmn=(1.+0.11*fmxfmn)
        vay_srband_fac=(1.+srband_fac)*.1
        vay_srb3=vay_srband_fac * vay_fmxfmn * eccentric

        pind=0.5*(f107+f107d) -80.
!

      do J = 1, NWAVES  
        jinv = nwaves+1-J
!
         if (idea_solar_fix.ge.1) then                               ! idea_solar_fix == 1 or 2
           flux(J)=sfmin(jinv)*max(0.8, (1.0+afac(jinv)*pind) )      !parameterized EUV 
         else              
           flux(J)=euv37(j)                                          ! solar_fix == 0   use observed EUV    
         endif
...
!
      IF (tau .ge.  1.e-36 .AND. tau <= 200.) then 
           attenuation = exp(-tau)
            ...
            attenuation=0.
      ENDIF
!   A tau of zero means all light gets through (none is absorbed attenuation = 1.

          local_flux=flux(J)*attenuation*eccentric

          IF(local_flux < 1.e-20 ) local_flux=0.

! EUV heating calculation(J/m3/s)
          IF(J <= NWAVES_EUV) THEN
! 
! For SI units convert flux to m^-2 (x10^4)   convert cross sections to m^2 (x10^-4)
!
...
!
! TIME-GCM JO2_LYA:
!             JLYA_loc= (6.0073e-21*exp(-8.2166e-21 *sco2(i))+
!     &                      4.28569e-21*exp(-1.63296e-20*sco2(i))+
!     &                      1.28059e-20*exp(-4.85121e-17*sco2(i)))*
!     &                      flux(lyman_a_num)
!
...
!==========================================================
       ...
       ELSE
              srband=O2(i)*1.e-6/(0.66*WO2+3.44E9*SQRT(WO2))! [17,20] in 60-100km Strobel-78 0.67 =>/0.66
       ENDIF
...
!
         shsrb(i) = srband*rnight_o2                  ! srbeff(i)=1.0 ML-93
         sh1(i)   = sh1(i)  *effeuv(i)                ! constant =0.05 in TIME-GCM
         shsrc(i) = shsrc(i)*effuv(i)                 !effuv(i)                 ! 0.80  !1(50km)=>0.75(80km)=>0.85 (110km) > ???
         shlya(i)  = shlya(i)*effuv(i)                ! 0.95
         sh2(i)   = shsrc(i) + shlya(i) +shsrb(i)     ! see SRC-fits in TGCM  ML-93, p10,527
! total
...
!  vay_fmxfmn*1.1E-7  =    5.56E-8  
!  Jsrb_TGCM= do2srb(k) = (5.56E-8*exp(-1.97E-10*sco2(k)**0.522))* [sfac*sfeps=vay_fmxfmn] [1/sec]
         ELSE
            JSRB_loc =vay_fmxfmn*1.45E8*(WO2**(-0.83))
         ENDIF
!
!        vay_fmxfmn*1.45E8 = 0.70E+8
!        Jsrb_tgcm(k) = (0.70E+8/(sco2(k)**0.83))*[sfac*sfeps=vay_fmxfmn] [1/sec]
! sum dissociation rates, eccentric is inclued in the local_flux
! 
       JSRC(i)   = JSRC_loc
       JSRB(i)   = JSRB_loc          
       JLYA(i)   = JLYA_loc
...
!
! see also
! /scratch3/NCEPDEV/swpc/save/Valery.Yudin/NCAR/TIMEGCM/timegcm_waccmx2016/timegcm_trunk/src/chapman.F
!  to compute sco2.... & vco2     
!=================================================================    
