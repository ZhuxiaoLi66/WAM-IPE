...
!     "inst_vis_dif_albedo                    ", &
  ...
  subroutine fillExportFields(data_a2oi, lonr, latr, rootPet, rc)
    ...
    do n=1, size(exportFields)
      if (ESMF_FieldIsCreated(exportFields(n))) then
        ...
        if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=153, file="module_CPLFIELDS.F90", rcToReturn=rc)) return
      endif
    enddo

    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=157, file="module_CPLFIELDS.F90", rcToReturn=rc)) return
  ...
  subroutine setupGauss2d(lonr, latr, pi, colrad_a, lats_node_a, &
    ...
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=190, file="module_CPLFIELDS.F90", rcToReturn=rc)) return
    
    call ESMF_VMGet(vm, petCount=petCount, rc=rc)
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=193, file="module_CPLFIELDS.F90", rcToReturn=rc)) return
    ...
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=199, file="module_CPLFIELDS.F90", rcToReturn=rc)) return
    ...
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=207, file="module_CPLFIELDS.F90", rcToReturn=rc)) return
    
    ! add coordinates    
    call ESMF_GridAddCoord(gauss2d, staggerLoc=ESMF_STAGGERLOC_CENTER, rc=rc)
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=211, file="module_CPLFIELDS.F90", rcToReturn=rc)) return

    call ESMF_GridAddCoord(gauss2d, staggerLoc=ESMF_STAGGERLOC_CORNER, rc=rc)
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=214, file="module_CPLFIELDS.F90", rcToReturn=rc)) return
    ...
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=217, file="module_CPLFIELDS.F90", rcToReturn=rc)) return
    ...
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=220, file="module_CPLFIELDS.F90", rcToReturn=rc)) return

    !--- CORNERS ---

    call ESMF_GridGetCoord(gauss2d, coordDim=1, staggerloc=ESMF_STAGGERLOC_CORNER, farrayPtr=lonCorPtr, rc=rc)
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=225, file="module_CPLFIELDS.F90", rcToReturn=rc)) return

    call ESMF_GridGetCoord(gauss2d, coordDim=2, staggerloc=ESMF_STAGGERLOC_CORNER, farrayPtr=latCorPtr, rc=rc)
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=228, file="module_CPLFIELDS.F90", rcToReturn=rc)) return
    
    write(tmpstr,'(a,4i8)') 'gsm gauss2d corner ',lbound(lonCorPtr,1),ubound(lonCorPtr,1),lbound(lonCorPtr,2),ubound(lonCorPtr,2)
    call ESMF_LogWrite(trim(tmpstr), ESMF_LOGMSG_INFO, line=231, file="module_CPLFIELDS.F90", rc=rc)
    ...
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=254, file="module_CPLFIELDS.F90", rcToReturn=rc)) return

    call ESMF_GridGetCoord(gauss2d, coordDim=2, farrayPtr=latPtr, rc=rc)
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=257, file="module_CPLFIELDS.F90", rcToReturn=rc)) return
    
    call ESMF_GridGetItem(gauss2d, itemflag=ESMF_GRIDITEM_MASK, farrayPtr=maskPtr, rc=rc)
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=260, file="module_CPLFIELDS.F90", rcToReturn=rc)) return

    call ESMF_GridGetItem(gauss2d, itemflag=ESMF_GRIDITEM_AREA, farrayPtr=areaPtr, rc=rc)
    if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=263, file="module_CPLFIELDS.F90", rcToReturn=rc)) return

    write(tmpstr,'(a,4i8)') 'gsm gauss2d center ',lbound(lonPtr,1),ubound(lonPtr,1),lbound(lonPtr,2),ubound(lonPtr,2)
    call ESMF_LogWrite(trim(tmpstr), ESMF_LOGMSG_INFO, line=266, file="module_CPLFIELDS.F90", rc=rc)
...
!      areaPtr(i,j) = abs(2.*pi/real(lonr) * cos(latPtr(i,j)*pi/180.) * pi/real(latr) * Rearth * Rearth)
  ...
  subroutine createWAMGrid(long, latg, levs, ipt_lats_node_a, lats_node_a, global_lats_a, &
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      ...
      if (ESMF_LogFoundError(rc, msg="Breaking out of subroutine", line=421, file="module_CPLFIELDS.F90", rcToReturn=rc)) return
  ...
  integer function queryFieldList(fieldlist, fieldname, abortflag, rc)
    ...
    if (labort .and. queryFieldList < 1) then
      call ESMF_LogWrite('queryFieldList ABORT on fieldname '//trim(fieldname), ESMF_LOGMSG_INFO, line=598, file="module_CPLFIELDS.F90", rc=rc)
